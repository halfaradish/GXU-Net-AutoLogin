...
- **基础配置**：校园网账号、密码、运营商
- **高级选项**（默认收起，点「高级选项 ▾」展开，开合状态会记住）：开机自启动、关闭窗口时最小化至托盘、程序启动后默认不弹窗、自定义日志目录（文件名固定为 `GXU_Net_AutoLogin.log`）、路由器 IP/MAC、MAC 地址（自动 / 从网卡列表选 / 手动输入）
...
- **托盘图标**：左键点击打开主界面；右键菜单为 显示主界面 / 立即重连 / 打开日志文件 / 打开日志目录 / 退出；断线、恢复、进入静默时会弹气泡提示
...
AUTOSTART=false          # 开机自启动
MINIMIZE_TO_TRAY=true    # 关闭窗口时最小化至托盘
START_MINIMIZED=true     # 启动后不弹窗（还没填账号时无论如何都会弹窗）
...
```bat
GXU_Net_AutoLogin_Tray.exe -show   :: 启动后直接显示主界面（忽略"启动后不弹窗"）
